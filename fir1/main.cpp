...
void ADC_Handler()
{
    ...
    static uint32_t Buffer_Insertion_Index = 0;
    ...
    if (ADC1->ISR & (1 << 3) )
    {
        ...
        delay_line[Buffer_Insertion_Index] = Chan2;                        
        float Output=0;            
        uint32_t Buffer_Read_Index = Buffer_Insertion_Index;
        
        
        for (int index=0;index < FILTER_LENGTH; index++)
        {
            Output += (float)delay_line[Buffer_Read_Index]*(float)b[index];
            /*
             * Assuming the filter is linear phase - i.e. symmetrical so can write it as follows:
             *
             * y = sum_from of k from 0 to N-1 of Buffer[k]*b[k]
             * 
             * Note : shunting not considered here.
             * 
             * 
            */
            Buffer_Read_Index = Buffer_Read_Index + 1;
            if (Buffer_Read_Index >= FILTER_LENGTH )
                Buffer_Read_Index = 0;            
        }
        DAC1->DAC_DHR12R1 = Output / (float)SCALE_FACTOR;
        DAC1->DAC_DHR12R2 = Output / (float)SCALE_FACTOR;
        ADC1->ISR = (1 << 3);     // clear ADC interrupt flag        
        
    }    

    Buffer_Insertion_Index++;
    if (Buffer_Insertion_Index >= FILTER_LENGTH) // wrap the insertion index if necessary
        Buffer_Insertion_Index = 0;
